...
def get_center_angle(left_coord: np.ndarray, right_coord: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    return np.float32(center_coord), np.float32(angle)
...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Get left and right finger coordinates and set them as keypoints
        ...
        # Apply the pipeline
        aug_rgb, aug_keypts = self.aug_pipeline(image = rgb, keypoints = keypts)
        
        # Get center point and angle 
        aug_center_point, aug_angle = get_center_angle(aug_keypts[0].xy, aug_keypts[1].xy)
        
        # Convert to Torch tensors and return
        data = {
            'rgb': aug_rgb,
            'center_point': np.array(aug_center_point, dtype=np.float32),
            'angle': np.array(aug_angle, dtype=np.float32)
        }
        data_torch = dict()
        for key, value in data.items():
            data_torch[key] = torch.from_numpy(value)
        ...
        """
        data_torch['rgb'] = torch.from_numpy(aug_rgb).to(torch.uint8)
        data_torch['center_point'] = torch.from_numpy(aug_center_point).type(torch.float32)
        data_torch['angle'] = torch.from_numpy(4).type(torch.float32)
        """
        return data_torch
...
def train(model, train_loader, criterion, optimizer, epoch, device):
    ...
    for cur_step, sample_batched in enumerate(train_loader):
        ...
        # Output stats every 50 iterations
        if cur_step % 50 == 0:
